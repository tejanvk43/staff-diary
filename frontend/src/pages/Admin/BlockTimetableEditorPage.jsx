...
import {
  Plus, Trash2, Edit2, X, Loader2, ArrowLeft, CalendarDays,
  Clock, Download, FileSpreadsheet, Printer,
} from 'lucide-react';
...
function SlotModal({ slot, fromTime, onClose, onSave }) {
  ...
  const [form, setForm] = useState({
    ...
    subject_type: slot?.subject_type === 'Break' ? 'Break' : 'Theory',
  });
  ...
  return (
    <div className="modal-overlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal-box" style={{ maxWidth: 380 }}>
        ...
        {/* Duration pill */}
        {dur > 0 && (
          <div style={{ display:'flex', alignItems:'center', gap:6, marginBottom:14, padding:'6px 12px', background:'var(--color-surface-2)', borderRadius:8, fontSize:'0.78rem' }}>
        ...
        <div className="form-group" style={{ marginBottom:14 }}>
          ...
          <select
            ...
            onChange={e => {
              ...
              if (val === 'Break' && !form.short_name) set('short_name', 'Break');
              else if (val === 'Theory' && form.short_name === 'Break') set('short_name', '');
            }}
          >
        ...
        <div className="form-group" style={{ marginBottom:22 }}>
          <label className="form-label" htmlFor="sl-label">
            Period Label <span style={{ fontWeight:400, color:'var(--color-text-muted)' }}>(optional)</span>
          </label>
          <input
            id="sl-label"
            className="input"
            ...
            placeholder="e.g.  P1,  P2,  Lunch Break…"
  ...
  return (
    <div style={{
      ...
      <div style={{ fontWeight:700, fontSize:'0.8rem', color:cardColor, paddingRight:44, lineHeight:1.2 }}>
        {label}{' '}
        {isBreak && (
          <span style={{ fontSize:'0.62rem', fontWeight:600, padding:'1px 4px', borderRadius:4, background:'rgba(245,158,11,0.15)', color:'#d97706', marginLeft:4 }}>
            Break
          </span>
        )}
      </div>
...
export default function BlockTimetableEditorPage() {
  ...
  // Save / update across ALL days for the same from_time row
  ...
  const exportToExcel = () => {
    ...
    const rows = uniqueTimes.map(timeStr => {
      ...
      const [fh2, fm2] = timeStr.split(':').map(Number);
      const [th2, tm2] = toStr ? toStr.split(':').map(Number) : [0,0];
      const dur2 = (th2*60+tm2) - (fh2*60+fm2);
      const dayVals = DAYS.map(day => {
        const s = getSlot(day, timeStr);
        if (!s) return '';
        return s.short_name || s.notes || (s.subject_type === 'Break' ? 'Break' : 'Period');
      });
      return [timeStr, toStr, dur2 > 0 ? dur2 : '', ...dayVals];
    });
      ...
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:20, flexWrap:'wrap', gap:12 }}>
        ...
        <div style={{ display:'flex', gap:10 }}>
          <div style={{ position:'relative' }}>
            ...
            {showExport && (
              <div style={{ position:'absolute', right:0, top:'110%', zIndex:50, background:'var(--color-surface)', border:'1px solid var(--color-border)', borderRadius:10, boxShadow:'0 8px 24px rgba(0,0,0,0.15)', minWidth:180, overflow:'hidden' }}>
                <button onClick={exportToExcel} style={EXPORT_BTN}>
                  <FileSpreadsheet size={14} style={{ color:'#10b981' }} /> Export as Excel
                </button>
                <button onClick={() => { window.print(); setShowExport(false); }} style={{ ...EXPORT_BTN, borderBottom:'none' }}>
      ...
      {loading ? (
        ...
        <div style={{ overflowX:'auto' }}>
          <table style={{ width:'100%', borderCollapse:'separate', borderSpacing:0, tableLayout:'fixed', minWidth:620 }}>
            ...
            <thead>
              <tr>
                <th style={{ padding:'12px 10px', background:'var(--color-surface-2)', border:'1px solid var(--color-border)', borderRight:'none', borderRadius:'10px 0 0 0', textAlign:'center', fontSize:'0.68rem', fontWeight:700, color:'var(--color-text-muted)', textTransform:'uppercase', letterSpacing:'0.08em' }}>
                  Time
                </th>
                {DAYS.map((day, i) => (
                  <th key={day} style={{ padding:'12px 8px', background:'var(--color-surface-2)', border:'1px solid var(--color-border)', borderLeft:'none', borderRadius: i === DAYS.length - 1 ? '0 10px 0 0' : 0, textAlign:'center' }}>
            ...
            <tbody>
              {uniqueTimes.length === 0 ? (
                <tr>
                  <td colSpan={DAYS.length + 1} style={{ padding:'56px 20px', textAlign:'center', border:'1px solid var(--color-border)', borderTop:'none', borderRadius:'0 0 10px 10px', background:'var(--color-bg)' }}>
                    <CalendarDays size={40} style={{ opacity:0.18, display:'block', margin:'0 auto 10px' }} />
                ...
                <>
                  {uniqueTimes.map((timeStr, rowIdx) => {
                    ...
                    return (
                      <tr key={timeStr}>
                        {/* Time label */}
                        <td style={{ padding:'8px', textAlign:'center', verticalAlign:'middle', background:'var(--color-surface-2)', border:'1px solid var(--color-border)', borderTop:'none', borderRight:'none', borderRadius: isLast ? '0 0 0 0' : 0 }}>
                        ...
                        {DAYS.map((day) => {
                          ...
                          return (
                            <td key={day}
                              ...
                              style={{ padding:6, verticalAlign:'top', border:'1px solid var(--color-border)', borderTop:'none', borderLeft:'none', background:'var(--color-bg)', minHeight:64, cursor:!cellSlot ? 'pointer' : 'default', transition:'background 0.12s' }}
                            ...
                            >
                              {cellSlot ? (
                                ...
                                <div className="add-hint" style={{ height:'100%', minHeight:52, display:'flex', alignItems:'center', justifyContent:'center', opacity:0, transition:'opacity 0.15s', color:'var(--color-text-muted)' }}>
                                  <Plus size={14} />
                                </div>
                              )}
                            </td>
                  ...
                  {/* Footer row to add another time slot */}
                  <tr>
                    <td colSpan={DAYS.length + 1} onClick={() => openAdd(null)}
                      style={{ padding:'10px', textAlign:'center', border:'1px solid var(--color-border)', borderTop:'none', borderRadius:'0 0 10px 10px', cursor:'pointer', color:'var(--color-text-muted)', fontSize:'0.78rem', fontWeight:600, transition:'background 0.12s' }}
